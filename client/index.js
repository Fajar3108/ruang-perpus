...
async function handleClickEditButton(bookId) {
  try {
   ...
   currentBook = await fetch(`http://localhost:3333/books/${bookId}`).then(response => response.json());
...
async function fetchBooks() {
  try {
    ...
    */
    books = await fetch('http://localhost:3333/books').then(response => response.json());
